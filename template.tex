...
{\color{blue}
Speed values, as shown in Figure \ref{shspeed}, tend to follow a similar pattern to distance values, $PV^*$ displaying the largest values and faster speeds in the SH than the NH.  $PV^*$-detected regions have the largest zonal speed values due to the combination of shorter duration and longer zonal distance, particularly in the SH; the 25 km/hr benchmark from Sinclair is exceeded more often by $PV^*$-detected regions than other methods. The relationship between $AGP$ and $Z^*$ consistently shows $AGP<Z^*$ in the SH, but the pattern is less clear in the NH, particularly in NH summer where $AGP$ detects a substantial number of low-latitude blocks with large distance and duration values, as noted in the previous sections.}
