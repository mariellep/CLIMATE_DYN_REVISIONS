...
{\color{teal}R2: The index by (Tibaldi and Molteni, 1990) is not calculated with respect to a time-varying central latitude. Please correct.}{\color{blue}MAR: Not in the original paper, but in the Barnes paper! }
...
The methods are briefly summarized in Table \ref{tabsum}, and explained in greater detail in the following sections. In order to differentiate between the methods from the original papers and the ones presented here, we use different abbreviations: our version of TM90 is $ZG$, DG83 is $Z^*$, and S04 is $PV^*$. 

\begin{table}
\caption{Summary of original methods and modifications. {\color{teal}R2: I do not see a difference between the ZG method as used here, and the AGP index used by (S. Scherrer and Appenzeller, 2006). I therefore suggest citing this paper here (with no changes to the original method except maybe the obvious sign changes for the Southern Hemisphere), and to also stick to the "AGP" name instead of ZG.}{\color{blue}MAR: I... er... checked this paper. I see no mention of AGP anywhere. The Scherrer et al paper which I cited in the text does indeed use the AGP label, so I suppose I could change the labels... all of them...}}
\label{tabsum}
\begin{tabular}{|L{0.12\textwidth}|L{0.22\textwidth}|L{0.22\textwidth}|L{0.22\textwidth}|}
\hline
\textbf{Method} &  \textbf{\citealt{tibaldi_operational_1990} ($ZG$)}  & \textbf{\citealt{dole_persistent_1983} ($Z^*$)}  &
...
\textbf{Change to original method}   & Extend analysis to all latitudes within 35-75$^{\circ}$, 5 days' persistence with contour overlap
...
\subsubsection{Geopotential Height Gradient (ZG)}\label{ghgdef}
...
TM90 performed these calculations about a single reference latitude band (60$^\circ$N), but the algorithm was modified by \cite{scherrer_two-dimensional_2006} to extend analysis to latitudes 35-75$^\circ$N and define $\phi_n$ and $\phi_s$ as 15$^\circ$ away from $\phi_0$. Furthermore, to apply this method in the SH, it is necessary to switch the criteria and signs for the two gradients, since  the orientation of ridges is flipped and the SH latitudes are negative. Therefore, in $ZG$, $GHGN<0$ m/deg lat and $GHGS>10$ m/deg lat in the SH. 
...
One of the most notable differences between methods is seen in the lower latitudes. The 2D blocking indices utilized here select certain blocking patterns that are typically missed by the 1D indices of \cite{tibaldi_operational_1990} and others, because  1D methods only identify blocks in the vicinity of a central latitude. Low-latitude blocks are the most obvious example of features that are missed by the 1D methods, and the largest blocking frequencies in the NH are all at the lower latitudes. These features will be discussed in further detail in Section \ref{lowlatsec}; while they tend to be less stationary than other blocking features, they still impair the zonal flow as per the AMS definition and are therefore worth including in the results.$ZG$ detects a high percentage of low-latitude blocks in NH summer (maximum frequency of $\approx 25\%$, or 22 days per season). These summer low-latitude blocks are not detected by the anomaly methods because deviation from the mean accounts for seasonally high values of their respective variables, but $ZG$ has no reference to the mean climatology. However, the anomaly methods do detect low-latitude blocks in winter (particularly in NP DJF), due to seasonally low values of the LTDM.
...
{\color{teal} R2: Table 3 onward: The statistical significance of these results (of differences between ZG, Z*, and PV*) needs to be established throughout.}
...
\hline
&\multicolumn{1}{|l|}{Season} & \multicolumn{1}{c|}{ZG} & \multicolumn{1}{c|}{$Z^*$} & \multicolumn{1}{c|}{$PV^*$} \\ \hline
...
\paragraph{Block zonal distance traveled:} As most previous blocking studies determine blocking on a per-gridpoint basis, it is difficult to draw direct comparisons to results here. Table \ref{dist} shows that, with the exception of NH summer, $ZG$ detected regions move the least zonal distance over time; regions detected by $Z^*$ have a range of values that is up to 50\% higher than the $ZG$ values and $PV^*$ values are greater still (up to twice as large as the values of the $ZG$ interquartile ranges), despite smaller  interquartile ranges for duration. The large values in NH summer for $ZG$ can be somewhat attributed to low-latitude blocks; 38\% of $ZG$-detected regions that are equatorward of 40 degrees (113 cases out of 301) have zonal distance measurements that exceed 2000 km ($\approx$ 24 degrees longitude) over the detected block's lifetime. Some of the larger zonal distance values for $PV^*$ can be attributed to the fact that, under certain meteorological conditions (see Section \ref{shearvort}), $PV^*$ detects gridpoints that are upstream relative to those detected by the other methods, meaning that the longitude coordinate of the cluster centroid is correspondingly shifted as well.
...
The resultant zonal distance between the start and end longitude coordinates can be hundreds of km greater than that of $Z^*$ or $ZG$, even if the three methods are tracking the same feature. Another factor involves $PV^*$-detected clusters that are embedded in regions of relatively unimpaired flow--- this is particularly prevalent at lower latitudes (see Section \ref{lowlatsec}).
...
When the speeds, shown in Table \ref{shspeed}, are calculated (as described in Appendix \ref{stitchdesc}), the 75th percentile values of the block speeds are overall higher in the SH, which is likely associated with the stronger zonal flow in the SH. $ZG$ detected regions have the lowest speeds (mainly due to shorter zonal distances), and while $Z^*$ detected regions had larger zonal distances, they had correspondingly larger duration values as well, which leads to a range of speeds that is in between $ZG$ and $PV^*$. $PV^*$ detected regions have the largest zonal speed values due to the combination of shorter duration and longer zonal distance, particularly in the SH; the range of values are upwards of 50\% larger those from other methods, and the 25 km/hr benchmark from Sinclair is exceeded more often by $PV^*$ detected regions than the other methods. 
...
\multirow{3}{*}{JJA} 
   & $PV^* $ and $ZG$  & 0.60 & \textit{0.30} & \textit{0.22} \\ 
   & $Z^*$ and $ZG$  & 0.65 & 0.51 & 0.37 \\  
   & $PV^*$ and $Z^*$  & \textbf{0.72} & \textbf{0.87} & \textbf{0.84} \\ 
   \hline
\multirow{3}{*}{DJF}
  & $PV^*$ and $ZG$ & \textbf{0.76} & 0.54 & 0.60 \\  
  & $Z^*$ and $ZG$  & 0.53& 0.38& \textbf{0.72}  \\ 
  & $PV^*$ and $Z^*$  & \textbf{0.75} & \textbf{0.87} & \textbf{0.77}  \\   \hline
\end{tabular}
...
\multirow{3}{*}{DJF}
  & $PV^*$ and $ZG$  & \textit{0.05}&\textit{ 0.13} & \textit{0.00}\\  
  & $Z^*$ and $ZG$   & \textit{0.21}& {0.42} & \textit{0.11}\\ 
  & $PV^*$ and $Z^*$  & \textbf{0.86} & \textbf{0.80} & \textbf{0.88} \\   \hline
  
  \multirow{3}{*}{JJA} 
   & $PV^* $ and $ZG$  & {0.49} & {0.53} & 0.56 \\ 
   & $Z^*$ and $ZG$  & {0.49} & {0.70} & {0.59} \\  
   & $PV^*$ and $Z^*$  & \textbf{0.93} & \textbf{0.92} & \textbf{0.96} \\ 
   \hline
\end{tabular}
\end{table}

\paragraph{Pearson pattern correlation:} The results in Table \ref{pearsontabcol} highlight the clear differences in blocking frequency that will arise given the choice of an anomaly versus a total field (discussed further in Section \ref{anominst}). Since each of the methods was initially constructed using NH DJF data, it is unsurprising that these correlation values  between all variable pairs are moderate to high in this subset of data, but this does not hold for other regions. Generally, correlation values are lower in the respective summer hemispheres. $C(PV^*, Z^*)$ is strong ($>0.71$) in all regions and seasons, but (with the exception of $C(Z^*, ZG)$ in NA DJF and $C(PV^*, ZG)$ in NC DJF) correlation between $ZG$ and the anomaly methods is low (0.00-0.30) to moderate (0.31-0.70).  
...
\multirow{8}{*}{JJA} 
   & \multirow{2}{*}{$PV^* $ and $ZG$}&$P(PV^*|ZG)$ & \textit{0.07} & \textit{0.08} & \textit{{0.09}}  \\  
   & & $P(ZG|PV^*)$ & {0.42} & {0.41} & {0.33}  \\ 
   &&&&&\\
   & \multirow{2}{*}{$Z^*$ and $ZG$} &$P(Z^*|ZG)$    & \textit{{0.23}} & \textit{{0.30}} & \textit{{0.30} } \\  
   &&$P(ZG|Z^*)$   & {0.39} & {0.56} & {0.39} \\ 
      &&&&&\\
      & \multirow{2}{*}{$PV^*$ and $Z^*$ } &$P(PV^*|Z^*)$    & \textit{0.15}& \textit{0.28}& \textit{{0.21}} \\ 
   &&$P(Z^*|PV^*)$   & {0.52} & \textbf{{0.74}} & 0.62  \\ 
  
  \hline
  \multirow{8}{*}{DJF} 
   & \multirow{2}{*}{$PV^* $ and $ZG$}&$P(PV^*|ZG)$ & {0.40} & {0.45} &\textit{{0.30}}  \\
   & & $P(ZG|PV^*)$& \textit{{0.29}} &\textit{0.30}& {0.45}\\ 
      &&&&&\\
   & \multirow{2}{*}{$Z^*$ and $ZG$} &$P(Z^*|ZG)$ & {0.67} &0.56& {0.65} \\ 
   &&$P(ZG|Z^*)$   & \textit{{0.18} }& \textit{0.23} & 0.38 \\ 
      &&&&&\\
      & \multirow{2}{*}{$PV^*$ and $Z^*$ } &$P(PV^*|Z^*)$    & \textit{0.23} & {0.48} & {0.30} \\
   &&$P(Z^*|PV^*)$   & {0.63} & \textbf{0.77} & \textbf{0.76}  \\   
  \hline
\end{tabular}
\subcaption{Southern Hemisphere}
\begin{tabular}{|l|l|l|l|l|l|l|l|l|l|}
\hline
  ...
  \hline
  \multirow{8}{*}{DJF} 
   & \multirow{2}{*}{$PV^* $ and $ZG$}&$P(PV^*|ZG)$ & \textit{{0.12}} & \textit{{0.17} }& \textit{{0.17}} \\
   & & $P(ZG|PV^*)$ & \textit{{0.02}} & \textit{0.11}& \textit{{0.01}} \\ 
      &&&&&\\
   & \multirow{2}{*}{$Z^*$ and $ZG$} &$P(Z^*|ZG)$  & \textbf{0.76} & \textbf{{0.73}} & \textbf{1.00} \\ 
   &&$P(ZG|Z^*)$  & \textit{{0.05} }& \textit{{0.20} }& \textit{{0.01} }\\ 
      &&&&&\\
      & \multirow{2}{*}{$PV^*$ and $Z^*$ } &$P(PV^*|Z^*)$    & {0.38} & {0.32} & {0.32} \\
   &&$P(Z^*|PV^*)$   &\textbf{0.80} & \textbf{0.78} & \textbf{0.80} \\   
   \hline
\multirow{8}{*}{JJA} 
   & \multirow{2}{*}{$PV^* $ and $ZG$}&$P(PV^*|ZG)$ & {0.49} & 0.26 & {0.65} \\  
   & & $P(ZG|PV^*)$  &\textit{{0.03}} & \textit{{0.17}} & \textit{{0.06}} \\ 
   &&&&&\\
   & \multirow{2}{*}{$Z^*$ and $ZG$} &$P(Z^*|ZG)$  & \textbf{0.98} & {0.66} & \textbf{0.79} \\  
   &&$P(ZG|Z^*)$    & \textit{{0.02}} & \textit{{0.18}} & \textit{{0.03}} \\ 
      &&&&&\\
      & \multirow{2}{*}{$PV^*$ and $Z^*$ } &$P(PV^*|Z^*)$    & \textit{{0.27}} & \textit{{0.30}}& \textit{{0.29}} \\ 
   &&$P(Z^*|PV^*)$   & \textbf{{0.73}} & {\textbf{0.73}} & \textbf{0.80} \\ 
   
  \hline
\end{tabular}
\end{table}

\paragraph{Probability of co-occurrence:} Here, the interpretation of values in Table \ref{probtabcol} must take the relative quantities of each method's detected features into account. $P(Z^*|ZG)$ is high in SI and SA JJA mainly because of the small number of $ZG$ detected regions with respect to the large number of $Z^*$ detected regions, so $ZG$ is a very good predictor of $Z^*$ but the reverse is not true ($P(ZG|Z^*)$ is often less than 0.20).  Note that despite the fact that $C(PV^*,ZG)\approx C(Z^*,ZG)$ during JJA in SI  and SA, $P(PV^*|ZG)$ is not similarly high; regions detected by these two methods rarely coincide. 

When this information is combined with the correlation coefficients from the previous section, it again reinforces the point that the anomaly methods are in higher agreement than $ZG$ and the anomaly methods. Although there are two instances of high correlation between $ZG$ and the anomaly methods ($C(PV^*, ZG)$ in NC JJA and $C(Z^*, ZG)$ in NA DJF), the probability of co-occurrence between these methods is only low to moderate; this means that the averaged pattern might appear to be similar, but the methods are not simultaneously detecting the same features on a per-timestep basis. In contrast, the high values of $P(Z^*|PV^*)$, combined with the high values of $C(PV^*, Z^*)$ indicates that $PV^*$ and $Z^*$ are much more likely to detect the same features (but $PV^*$ will do so less often, given the lower blocking frequency).% {\color{blue}[MAR: another thing to add to conclusion]}
...
\multirow{3}{*}{JJA} 
   & $PV^* $ and $ZG$    & {0.26} to {0.46} & \textit{0.10} to \textit{0.39} & {0.20} to \textit{0.45}  \\  
   & $Z^*$ and $ZG$   & \textbf{0.36} to \textbf{0.58} & {0.20} to {0.43} & {0.25} to {0.52}  \\   
   & $PV^*$ and $Z^*$   & \textbf{0.34} to \textbf{0.56} & \textbf{0.32} to \textbf{0.54} & \textbf{0.31} to \textbf{0.55}  \\ 
   \hline
\multirow{3}{*}{DJF}
  & $PV^*$ and $ZG$ & \textbf{0.29} to \textbf{0.54} & {0.22} to {0.45} & \textit{0.16} to {0.44} \\   
  & $Z^*$ and $ZG$ & \textbf{0.34} to \textbf{0.56} & {0.18} to {0.42} & 0.25 to {0.50} \\ 
...
\multirow{3}{*}{DJF}
  & $PV^*$ and $ZG$  & \textit{0.04} to \textit{0.28} & \textit{0.09} to \textit{0.38} & \textit{0.06} to \textit{0.13} \\   
  & $Z^*$ and $ZG$ & \textit{0.14} to \textit{0.27} & {0.20} to \textit{0.38} & {0.23} to \textit{0.29} \\ 
  & $PV^*$ and $Z^*$  & \textbf{0.36} to \textbf{0.59} & \textbf{0.34} to \textbf{0.61} & \textbf{0.38} to \textbf{0.59} \\  
  \hline
  \multirow{3}{*}{JJA} 
   & $PV^* $ and $ZG$    & \textit{0.15} to \textit{0.30} & \textit{0.11} to \textit{0.34} & \textit{0.11} to \textit{0.37} \\  
   & $Z^*$ and $ZG$   & {0.21} to \textit{0.38} & {0.18} to {0.40} & {0.20} to \textit{0.31} \\   
...
\hline
\multirow{2}{*}{$PV^*$ and $ZG$} & \multirow{2}{*}{0.38} & $P(PV^*|ZG)$: 0.44 & \multirow{2}{*}{0.21 to 0.51} \\
& & $P(ZG | PV^*)$: \textit{0.24 }& \\
& & & \\
\multirow{2}{*}{$Z^*$ and $ZG$} & \multirow{2}{*}{0.41} & $P(Z^*|ZG)$: \textbf{0.76} & \multirow{2}{*}{0.16 to 0.40} \\
& & $P(ZG | Z^*)$: \textit{0.19} & \\
& & & \\
...
In Section \ref{intercompare}, the magnitudes of the agreement metrics show that there is a clear distinction between circumstances when blocks are detected by $ZG$, a method based on the total $Z500$ field, as opposed to the other two methods that are based on anomalies with respect to the LTDM. We limit the discussion here to the two methods that are based on $Z500$ in order to reduce other sources of variability.

Both $Z^*$ and $ZG$ were created with the purpose of detecting high values of $Z500$, but $ZG$ has no reference to the mean climatology; instead, it requires a significant change in $Z500$ over a latitude range (30 degrees) that equates to more than half of each study region's latitude range (50 degrees). The difference in block detection is most evident in the SH, where there is a much stronger zonal flow component than in the NH. This stronger zonal flow implies that the requirements of the $ZG$ method are fulfilled far less often in the SH, since there is not sufficient distortion of the flow field; $ZG$-detected blocks in the SH are often co-located with lows (dipole or omega blocks) or have a tilt in their north-south axis, since this guarantees that there is a sufficiently large $Z500$ gradient. In contrast, $Z^*$ is much less discriminating; it detects a high number of blocks in the SH because the $\overline{Z500}$ field has a strong meridional gradient in addition to its mainly zonal flow, meaning that even fairly shallow ridges in the $Z500$ field are more likely to satisfy $Z^*$ anomaly thresholds. 

These points are illustrated in Figure \ref{zgdiff}, which shows a snapshot of an omega block in SP on May 5th 18Z 1998 (left) and 12 hours later (right). At the earlier time, the two anomaly methods produce clusters that are centered over the ridge ($S(PV^*, Z^*)=0.78$), but the $ZG$ method only picks out an area centered polewards of the lower geopotential heights to the east of the ridge (before the size constraint was applied, there was also a detected region in the vicinity of 190E,65S); $S(PV^*, ZG)=0.24$ and $S(Z^*, ZG)=0.26$. Later, the intensification of the high in the 60-70S latitudes produces the necessary height gradient to satisfy the criteria for $ZG$; $S(PV^*, ZG)=0.46$ and $S(Z^*, ZG)=0.58$. 
...
& Low to moderate for both $P(PV^*|ZG)$ and $P(ZG|PV^*)$
& Moderate to high $P(Z^*|ZG)$ when many $Z^*$, low otherwise
...
\caption{(a-b) Examples of ridges that were detected by only the anomaly methods ($Z^*$, blue, and $PV^*$, green) in January 2013. (c-d) Examples of ridges that were detected by all three methods (including $ZG$, purple) in December of 2013.}\label{RRRpanel}
\end{figure*}
...
\caption{Example, 12 hours apart in 1995 MAM, of instances in which there is (a) less and (b) more agreement between the $ZG$ method (purple) and the two anomaly methods (blue and green) in SP.}\label{zgdiff}
\end{figure*}
...
\caption{Example, in 24-hour increments, of omega block detection in 1989 NA SON. The $PV^*$ method is denoted by the green contour, the $Z^*$ method is blue, and the $ZG$ method is purple.}\label{pvdiff}
\end{figure*}
